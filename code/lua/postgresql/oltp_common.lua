...
local function get_id()
   local thread_id = sysbench.tid -- 当前线程 ID
   local thread_count = sysbench.opt.threads -- 总线程数
   local id_per_thread = math.ceil(sysbench.opt.table_size / thread_count) -- 每个线程的 ID 范围大小

   local start_id = id_per_thread * thread_id + 1 -- 当前线程的起始 ID
   local end_id = start_id + id_per_thread - 1   -- 当前线程的结束 ID
